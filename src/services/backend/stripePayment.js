...
export const payWithStripe = async (paymentMethod, payableAmount) => {
    ...
    await createToken(params),
        then(async token => {
            if (token.tokenId) {
                console.log('token created', token) // total buy price
                ...
                await charge(token.tokenId, total_buy_price).
                    then(async (data) => {
                        ...
                        if (data.amount && data.amount_captured) {
                            ...
                            Toasts.simple('Transaction failed, please try again')
                        }
                        else {
                            Toasts.simple('Transaction failed, please try again')
                        }
                    ...
                    .catch((err) => {
                        ...
                        Toasts.simple('Could not access your card at this time')
                    })
            }
        }).
        catch(err => {
            Toasts.simple('Invalid payment details.')
        })
...
export const createToken = async ({number,expMonth,expYear,cvc}) => {
    ...
    let data = await fetch('https://api.stripe.com/v1/' + 'tokens', {
        headers: {
            ...
            //Authorization: stripeKeys.authorizationKey,
