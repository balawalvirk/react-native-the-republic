...
function Find({navigation, route}) {
  ...
  const [filteredProductsCurrentPage, setFilteredProductsCurrentPage] =
    useState(1);
  const [loading, setLoading] = useState(true);
  const [loadingMore, setLoadingMore] = useState(false);
  const [filteredProductsLoadingMore, setFilteredProductsLoadingMore] =
    useState(false);
  const [allItemsLoaded, setAllItemsLoaded] = useState(false);
  const [allFilteredProductsLoaded, setAllFilteredProductsLoaded] =
    useState(false);
  ...
  const getSetAllProducts = async data => {
    ...
    await Backend.getAllProducts({sort_by: sortBy, page: tempPage}).then(
      res => {
        if (res) {
  ...
  const getSetFilteredProducts = async data => {
    ...
    const tempInitialData = data?.initialData || null;
    await Backend.filterProducts({...filterData, sortBy, page: tempPage}).then(
      res => {
        if (res) {
  ...
  const getSetMapProducts = async () => {
    await Backend.getMapProducts().then(res => {
      if (res) {
  ...
  const getMarkers = () => {
    let tempMarkers = [];
    tempMarkers = mapProducts.filter(
      item => item.latitude && item.longitude && item.user,
    );
    return tempMarkers;
  };
        ...
        <View
          ...
          <Wrapper
            ...
            style={{
              ...
              borderTopRightRadius: sizes.cardRadius * 2,
              borderTopLeftRadius: sizes.cardRadius * 2,
            ...
            {selectedProduct ? (
              <>
                <ProductCardPrimary
                  isFavourite={HelpingMethods.checkIsProductFavourite(
                    selectedProduct.id,
                  )}
                  viewType={'list'}
                  image={
  ...
  return (
    <MainWrapper>
      {selectedTabIndex === 0 ? (
        <Products
          ...
          onPressProduct={(item, index) =>
            navigate(routes.productDetail, {product: item})
          }
          ...
          isLoading={loading}
          isLoadingMore={
            !filteredProducts ? loadingMore : filteredProductsLoadingMore
          }
          onEndReached={data => {
          ...
          {getMarkers().map((item, key) => {
            ...
            return (
              <Marker
                ...
                <CustomIcon icon={appIcons.map_pin} size={totalSize(5)} />
              </Marker>
            );
          })}
        </MapView>
        ...
        <SkeletonPrimary
          itemStyle={{height: height(100), marginHorizontal: 0}}
        />
      )}
      ...
      <AbsoluteWrapper
        style={{bottom: sizes.marginVertical, right: 0, left: 0}}>
        <Wrapper
          style={[{height: height(5), alignSelf: 'center'}, appStyles.center]}>
          <ButtonGroupAnimated
            data={bottomTabs}
      ...
      <BottomSheetModal
        ref={sheetRef}
        index={0}
        snapPoints={[height(35), height(80)]}
        // borderRadius={sizes.cardRadius*2}
      >
        {renderContent}
      </BottomSheetModal>
