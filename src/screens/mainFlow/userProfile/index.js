...
function UserProfile(props) {
  ...
  const isMyProfile = useMemo(() => {
    return myProfileDetails?.id === userDetail?.id;
  }, [myProfileDetails, userDetail]);
  ...
  const getSetData = async () => {
    if (userId) {
      await Backend.getUserProfileDetail(userId).then(res => {
        if (res) {
  ...
  const handleGetUserProducts = async () => {
    await Backend.get_user_products({userId: user_id, page: 1}).then(res => {
      if (res) {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Wrapper>
          <Wrapper style={[appStyles.center]}>
            <ImageProfile
              source={{
                uri: userDetail.profile_photo_path
            ...
            {userDetail.address ? (
              <>
                <RegularText style={[appStyles.textLightGray]}>
                  {userDetail.address}
                </RegularText>
                <Spacer height={sizes.smallMargin} />
              </>
            ) : null}
{
    ...
    <>
      <ButtonColoredSmall
              text={
                HelpingMethods.checkIfFollowRequestSent(user_id)
                  ? 'Request Sent'
                  : HelpingMethods.checkIfFollowingUser(user_id)
                  ? 'Following'
                  : 'Follow'
              }
              textStyle={[
              ...
              buttonStyle={[
                {paddingHorizontal: sizes.marginHorizontalXLarge},
                HelpingMethods.checkIfFollowRequestSent(user_id)
                  ? {
    ...
    null
            ...
            onPressPost={(item, index) =>
              navigate(routes.postDetail, {
                ...
                updateData: ({updatedPost, deletePost}) => {
                  updatedPost
                    ? setUserPosts(
                        HelpingMethods.handleReplacePost(
                          userPosts,
                          updatedPost,
                        ),
                      )
                    : deletePost
                    ? setUserPosts(
                        HelpingMethods.handleRemovePost(userPosts, deletePost),
                      )
                    : null;
                },
              })
      ...
      <BackIconAbsolute onPress={goBack} />
    </MainWrapper>
  );
}
