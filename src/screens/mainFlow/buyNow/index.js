...
function BuyNow(props) {
  ...
  if (delivery_address) {
    ...
    deliveryAddress =
      house + ', ' + street + ', ' + city + ', ' + zip_code + ', ' + state;
  }
  //local states
  ...
  const [isOrderPlacedPopupVisible, setOrderPlacedPopupVisible] =
    useState(false);

  const toggleOrderPlacedPopup = () =>
    setOrderPlacedPopupVisible(!isOrderPlacedPopupVisible);
  ...
  useEffect(() => {
    ...
    if (default_card_id && creditCards.length) {
      const tempCreditCard = creditCards.find(
        item => item.id.toString() === default_card_id,
      );
  ...
  const getSetSellerCopuns = () => {
    Backend.getUserCoupons(user.id).then(res => {
      if (res) {
        setCoupons(res.data);
      }
    });
  ...
  const handleOnChangeCouponText = text => {
    ...
    if (coupons.length) {
      const tempOBj = coupons.find(
        item => item.code.toLowerCase() === couponQuery,
      );
      if (tempOBj) {
  ...
  const getDiscountAmount = () => {
    let discount = 0;
    if (coupon) {
      const {discount_type, discount_amount, code, minimum_order, expiry_date} =
        coupon;
  ...
  const transectionCharges = HelpingMethods.getRoundedValue(
    (subTotal / 100) * 2,
  );
  const total = HelpingMethods.getRoundedValue(
    subTotal + tax + transectionCharges,
  );
  ...
  const handleBuyNow = async () => {
    if (validations()) {
      try {
        Toasts.error('Please Add/Select Payment Method');
        setLoadingBuy(true);
        // const payWithStripeRes = await Backend.payWithStripe(
        //   defaultCreditCard,
        //   total,
        // );
        // if (!payWithStripeRes) {
        ...
        //   return;
        // }
        //const stripeCargeId = payWithStripeRes.id
        const stripeCargeId = 'jhagsduytas8d76';
    ...
    <MainWrapper>
      <KeyboardAvoidingScrollView>
        ...
        <ProductCardSecondary
          ...
          moreInfoTitle={
            user ? user.first_name + ' ' + user.last_name : 'Anonymouse'
          }
        ...
        />
        <Spacer height={sizes.baseMargin} />
        <TitleValue title={'Tax (10%)'} value={'$ ' + tax} />
        <Spacer height={sizes.baseMargin} />
        <TitleValue
            ...
            {getDiscountAmount() === 0 ? (
              <ComponentWrapper style={{marginRight: width(30)}}>
                ...
                <SmallText style={[appStyles.textGray]}>
                  Not Applicable on this order, minimum order should be{' '}
                  <SmallText style={[appStyles.fontBold]}>
        ...
        <Wrapper
          style={[
            appStyles.grayWrapper,
            {paddingVertical: sizes.baseMargin * 1.5},
          ]}>
          <RowWrapperBasic>
            <Wrapper flex={1}>
            ...
            <Wrapper flex={1}>
              ...
              {defaultCreditCard ? (
                <>
                  ...
                  <MediumText>
                    {HelpingMethods.getHiddenCardNumber(
                      defaultCreditCard.card_number,
                    )}
                  </MediumText>
                </>
              ) : null}
        ...
        <Wrapper style={[appStyles.grayWrapper, {}]}>
          <RowWrapperBasic>
            ...
            <Wrapper flex={1} style={{alignItems: 'flex-end'}}>
              <TinyTitle
                onPress={() => navigate(routes.deliveryAddress)}
                style={[appStyles.textPrimaryColor]}>
                {delivery_address ? 'Change' : 'Add'}
              </TinyTitle>
            </Wrapper>
          </RowWrapperBasic>
        </Wrapper>
        ...
        <Wrapper style={[appStyles.borderedWrapper, {}]}>
          ...
          <Spacer height={sizes.baseMargin} />
          <RegularText>
            If you and the seller are in the same area and don't want to involve
            a FFL Dealer, then you might enable this
          </RegularText>
          {!privateSale ? (
            <Wrapper>
              <Spacer height={sizes.baseMargin} />
              <UserCardPrimary
                ...
                imageUri={
                  default_dealer
                    ? default_dealer.profile_photo_path
                    : appImages.noUser
                }
                title={
                  default_dealer
                    ? default_dealer.first_name + ' ' + default_dealer.last_name
                    : 'Dealer'
                }
                subTitle={default_dealer_id ? '3 miles away' : ''}
                right={
                  <TinyTitle
                    onPress={() => navigate(routes.fflDealers)}
                    style={[appStyles.textPrimaryColor]}>
                    {default_dealer ? 'Change' : 'Select'}
                  </TinyTitle>
                }
              />
            </Wrapper>
      ...
      <PopupPrimary
        ...
        title="Order Placed"
        info={
          "You'll be notified when your order is accepted and on it's way to delivery." +
          '\n\n' +
