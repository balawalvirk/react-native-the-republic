...
function UpgradeSubscriptionPlan(props) {
  ...
  const [loadingCancelSubscription, setLoadingCancelSubscription] =
    useState(false);
  ...
  const [
    isSubscriptionUpgradedPopupVisible,
    setSubscriptionUpgradedPopupVisible,
  ] = useState(false);
  const [
    isCancelCurrentSubscriptionPopupVisible,
    setCancelCurrentSubscriptionPopupVisible,
  ] = useState(false);
  const toggleSubscriptionUpgradedPopup = () =>
    setSubscriptionUpgradedPopupVisible(!isSubscriptionUpgradedPopupVisible);
  const toggleCancelCurrentSubscriptionPopup = () =>
    setCancelCurrentSubscriptionPopupVisible(
      !isCancelCurrentSubscriptionPopupVisible,
    );
  ...
  const handleSetupSubscription = () => {
    ...
    try {
      Backend.update_profile({
        customer_id: 'aasdasdas235324rwd',
        payment_id: '7a6sd876as8d',
        subscription_id: 't6a8s7d678asd68',
      ...
      console.log('Error in handleSetupSubscription', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCancelCurrentSubscription = async () => {
   ...
   try {
    ...
    console.log('Error in handleCancelCurrentSubscription', error);
   } finally {
...
//     await Backend.cancelStripeSubscribtion(subscription_id).then(
//       async response => {
...
//   };
  ...
  return (
    <MainWrapper>
      ...
      <Spacer height={sizes.baseMargin} />
      <TitleValue title={'Subtotal'} value={'$ ' + subTotal} />
      <Spacer height={sizes.baseMargin} />
